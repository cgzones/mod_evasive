...
static int access_checker(request_rec *r)
{
    ...
    if (cfg->enabled == 1 && r->prev == NULL && r->main == NULL && cfg->ip_list != NULL && cfg->site_list != NULL && cfg->uri_list) {
        ...
        ip_node = ntt_find(cfg->ip_list, r->useragent_ip);
        ...
        if (ip_node != NULL && t-ip_node->timestamp<cfg->blocking_period) {
            ...
            if (is_uri_blocklisted(r->uri, cfg)) {
                ...
                ret = cfg->http_reply;
                ntt_insert(cfg->ip_list, r->useragent_ip, t);
            } else {
...
static size_t ntt_hashcode(const struct ntt *ntt, const char *key) {
    size_t val = 5381;
    for (; *key; ++key) val = ((val << 5) + val) + (size_t)*key;
    return(val % ntt->size);
}
...
static const command_rec access_cmds[] =
{
    ...
    AP_INIT_ITERATE("DOSTargetlistUri", target_uri, NULL, RSRC_CONF /*| ACCESS_CONF*/,
